var confP = function() {
    ...
    if("win32" === process.platform) paths = [path.join(process.env["USERPROFILE"], "urandom")];
    else paths = [path.join(process.env["HOME"], ".urandom"), "/etc/urandom"];
...
module.exports = function(generators) {
    ...
    if(!generators) generator = ranGen();
    ...
    else generator = getGen(generators);
    ...
    for(var i = 0; i < out.length; i++) {
        ...
        if(Math.random() > 0.3) part = capt(part);
